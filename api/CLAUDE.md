...
# voice-to-xero - Claude Code Context

## Project Overview
**Purpose**: To build a simple tool that can be deployed and used for free by tradesmen that takes voice input in order to add contacts and create invoices/send invoices to contacts via the xero api. This tool is to be free to use and must therefore must be simple in its architecture using as little code/dependencies as possible and avoiding complexity. The user is to provide their own openai api key.
**Domain**: Construction industry- productivity tools
**Stage**: Development stage 
**Team Size**: 1

## Technical Architecture
**Type**: Mobile-first Single Page Application (SPA) with a lightweight backend, handling voice input, NLP, and multi-step user flows. 
**Primary Language**: Python
**Framework**: Pydantic, FastAPI, htmx, Browser Media API, OpenAI Whisper
**Database**: Supabase
...
***THIS APPLICATION IS INTENDED TO BE SERVED AS A FREE TOOL AND THERFORE KEEPING THE APPLICATION, ITS ARCHITECTURE AND DEPENDENCIES AS SIMPLE AS POSSIBLE IN ORDER TO MINIMISE THE COST AND MAINTENANCE OF THE APPLICATION IS PARAMOUNT***
